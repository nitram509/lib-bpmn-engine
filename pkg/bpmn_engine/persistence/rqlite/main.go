...
type RqliteContext struct {
	httpServ    *httpd.Service
	cfg         *Config
	clstrClient *cluster.Client
	credStr     *auth.CredentialsStore
	Str         *store.Store
	muxLn       net.Listener
...
func Start() *RqliteContext {
	...
	return &RqliteContext{httpServ, cfg, clstrClient, credStr, str, muxLn, clstrServ, mainCtx}
}

func Stop(rcontext *RqliteContext) {

	// Stop the HTTP server first, so clients get notification as soon as
	// possible that the node is going away.
	rcontext.httpServ.Close()

	if rcontext.cfg.RaftClusterRemoveOnShutdown {
		remover := cluster.NewRemover(rcontext.clstrClient, 5*time.Second, rcontext.Str)
		remover.SetCredentials(cluster.CredentialsFor(rcontext.credStr, rcontext.cfg.JoinAs))
		log.Printf("initiating removal of this node from cluster before shutdown")
		if err := remover.Do(rcontext.cfg.NodeID, true); err != nil {
	...
	if rcontext.cfg.RaftStepdownOnShutdown {
		if rcontext.Str.IsLeader() {
		...
		rcontext.Str.Stepdown(true)
	}

	if err := rcontext.Str.Close(true); err != nil {
		log.Printf("failed to close store: %s", err.Error())
	}
	rcontext.clstrServ.Close()
	rcontext.muxLn.Close()
	stopProfile()
