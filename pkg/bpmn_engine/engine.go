...
func (state *BpmnEngineState) handleElement(process BPMN20.ProcessElement, act activity, instance *processInstanceInfo, element *BPMN20.BaseElement, originActivity activity) []command {
	...
	case BPMN20.ServiceTask:
		...
		} else if job.ErrorEvent != nil {
			// The current process will remain ACTIVE until the event sub-processes have completed.
			nextCommands = handleErrorEvent(process, instance, element, *job.ErrorEvent)
...
func handleErrorEvent(process BPMN20.ProcessElement, instance *processInstanceInfo, element *BPMN20.BaseElement, event ErrorEvent) []command {
	// Find the error by code on the process
	if errT, found := findErrorDefinition(instance.ProcessInfo.definitions, event.Code); found {
		...
		return []command{
			errorCommand{
				err:         newEngineErrorf("Could not find suitable handler for Error event id=%s, code=%s", errT.Id, errT.ErrorCode),
				elementId:   (*element).GetId(),
				elementName: (*element).GetName(),
			},
		}
		...
		return []command{
			errorCommand{
				err:         newEngineErrorf("Could not find error definition \"%s\"", event.Code),
				elementId:   (*element).GetId(),
				elementName: (*element).GetName(),
			},
		}
